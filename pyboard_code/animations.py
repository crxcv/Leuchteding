...
def blink(color=random.randint(0, 0xffffff), count=3,
          time_on=200, time_off=100):
    print("blink")
...
def running_dot(color=0x00FFFF, time_on=60):
    print("running_dot")
    while True:
...
def ripple():
    ...
    while True:
        ...
        if step is 0:
            ...
            if step < maxSteps:
                strip.setHSB(wrap(center + step), color, 255,
                             pow(fadeRate, step)*255, update=False)
                if step > 3:
                    strip.setHSB(wrap(center + step - 3), color, 255,
                                 pow(fadeRate, step - 2)*255, update=False)
                    strip.setHSB(wrap(center - step + 3), color, 255,
                                 pow(fadeRate, step - 2)*255, update=False)
                step += 1
            else:
...
def sparkle():
    ...
    while True:
        for i in range(1, num_pixels+1):
            ...
            RGB = []
            RGB = [int(x) for x in val]
            color = int("0x"+"".join(["0{0:x}".format(v) if v < 16
...
animation_dict = {"blink": blink, "running_dot": running_dot,
                  ...
                  "fire": fire, "wave": wave, "ripple": ripple,
                  "sparkle": sparkle, "off": leds_off}
...
def start(animation_name):
    """ starts new animation saved in animation_dict by key argument given by
    animation_name
